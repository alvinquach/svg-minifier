...
export class SvgParserService {
    ...
    parse(svg: string): SvgObject {
        ...
        let svgElements: string[] = svg.split("><");
        ...
        let firstElement: string = svgElements[0];
        if (firstElement.indexOf("<") == 0) {
            svgElements[0] = firstElement.substring(1);
        }
        let lastElement: string = svgElements[svgElements.length - 1];
        ...
        let root: SvgObject = new SvgObject("root", svgElements);
